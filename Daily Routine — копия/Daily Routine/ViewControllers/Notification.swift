...
class Notifications: NSObject, UNUserNotificationCenterDelegate {
    ...
    func pushNotification(tasks countTasts: Int) {
        ...
        let formTasks = countTasts == 1 ? "task" : "tasks"
        let message = "scheduled for today aren't done"
        notificationContent.body = "\(countTasts) \(formTasks) \(message)"
